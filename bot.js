...
====================== */
/* =====================
   AI COMMANDS (FIXED - LOGIKA HTML)
====================== */
if (text?.startsWith(".ai ") || text?.startsWith(".gemini ")) {
  const q = text.replace(/^\.(ai|gemini)\s*/, "");
  if (!q) return send(chatIdMsg, "❌ Format salah! Ketik: `.ai <pertanyaan>`");
  
  await send(chatIdMsg, "bentar ya 🤓..");

  try {
    const apiUrl = 'https://api.ryzumi.vip/api/ai/gemini';
    
    // LOGIKA PARAMS (SAMA DENGAN HTML)
    ...
    const response = await fetch(`${apiUrl}?${params.toString()}`, {
      ...
      headers: { 
        ...
        // WAJIB: Biar gak dapet Error 403/500 (Identitas Browser)
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'
      }
    });

    // Cek status HTTP (untuk log error 403/500)
    if (!response.ok) {
      console.error(`API Error: ${response.status}`);
      return send(chatIdMsg, `⚠️ Server API Error ${response.status}. Coba lagi nanti.`);
    }
    ...
    try {
      ...
      // Jika masuk sini, berarti API ngirim HTML (Gangguan/Maintenance)
      return send(chatIdMsg, "⚠️ Server API sedang maintenance (Mengirim HTML).");
    }

    // LOGIKA PENGAMBILAN DATA (SAMA DENGAN HTML)
    let aiReply = (data.result || data.success) ? data.result : 'Maaf, saya tidak mengerti.';
    
    // LOGIKA PEMBERSIHAN (SAMA DENGAN HTML)
    ...
    return send(chatIdMsg, "❌ Gagal terhubung ke API. Cek koneksi server.");
  }
}
  /* =====================
     REMINI (FIXED)
