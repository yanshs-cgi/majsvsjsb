...
/* =====================
   AI COMMANDS (FIXED - LOGIKA SAMA DENGAN HTML)
====================== */
if (text?.startsWith(".ai ") || text?.startsWith(".gemini ")) {
  const q = text.replace(/^\.(ai|gemini)\s*/, "");
  if (!q) return send(chatIdMsg, "❌ Format salah! Ketik: `.ai Apa kabar?`Requested content");
  
  await send(chatIdMsg, "🔄 Sedang berpikir...");

  try {
    // 1. Logic fetch disamakan dengan HTML (menggunakan encodeURIComponent)
    const apiUrl = `https://api.ryzumi.vip/api/ai/gemini?text=${encodeURIComponent(q)}`;
    ...
    // 2. Proteksi agar tidak error "DOCTYPE HTML" jika API down
    const contentType = response.headers.get("content-type");
    if (!response.ok || (contentType && contentType.includes("text/html"))) {
      return send(chatIdMsg, "⚠️ Server API Ryzumi sedang gangguan/down (Mengirim HTML). Coba lagi nanti.");
    }

    const data = await response.json();

    // 3. Ambil hasil (sama dengan logika data.result di HTML kamu)
    let aiReply = (data.result || data.success) ? data.result : 'Maaf, saya tidak mengerti.';
    
    // 4. Pembersihan teks (Sama dengan logika .replace di HTML kamu)
    aiReply = aiReply.replace(/prompt:/gi, '').replace(/result:/gi, '').trim();

    // 5. Kirim balik ke Telegram
    return send(chatIdMsg, `🤖 *AI Response:*\n\n${aiReply}`);

  } catch (error) {
    console.error("Error di Fitur AI:", error);
    return send(chatIdMsg, "❌ Gagal terhubung ke API. Pastikan jaringan server aman.");
  }
}
