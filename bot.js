...
/* =====================
   AI COMMANDS (ANTI 403 - BRUTE FORCED HEADERS)
====================== */
if (text?.startsWith(".ai ") || text?.startsWith(".gemini ")) {
  ...
  try {
    ...
    // Kalau dapet 403 lagi, berarti IP server lo diblokir permanen sama Ryzumi
    if (response.status === 403) {
      return send(chatIdMsg, "❌ Error 403: Akses ditolak Cloudflare. IP Server bot lo diblokir sama API Ryzumi.");
    }

    if (!response.ok) {
      return send(chatIdMsg, `⚠️ API Error ${response.status}.`);
    }

    const data = await response.json();
    let aiReply = (data.result || data.success) ? data.result : 'Maaf, saya tidak mengerti.';
    
    // Pembersihan teks sesuai logika HTML lo
